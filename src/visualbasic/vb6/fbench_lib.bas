Attribute VB_Name = "modFBLIB"
Option Explicit

'  VB6 inntrinsic trig functions are used when INTRIG is true.  The
...
Public Const piover2 = pi / 2#

Const twopi = pi * 2#
Const piover4 = pi / 4#
Const fouroverpi = 4# / pi
...
Function aInt(ByVal x As Double) As Double
   ...
   '
   l = x
   If (CLng(-0.5) <> 0) And (l < 0) Then l = l + 1
   aInt = l
   End Function
...
Function fSin(ByVal x As Double) As Double
   ...
   '
   Dim sign As Boolean
   Dim Y As Double, r As Double, z As Double
...
Function fSqrt(ByVal x As Double) As Double
   ...
   Dim C As Double, cl As Double, Y As Double
   Dim n As Long
...
atl:
   z = x * x
   b = ((((893025# * z + 49116375#) * z + 425675250#) * z + _
       1277025750#) * z + 1550674125#) * z + 654729075#
   a = (((13852575# * z + 216602100#) * z + 891080190#) * z + _
       1332431100#) * z + 654729075#
   a = (a / b) * x + atanc(Y)
...
Function aTan2(ByVal Y As Double, x As Double) As Double
   '
