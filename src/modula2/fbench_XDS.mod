...
compiler is Excelsior XDS Modula-2 v2.60 beta,
from J.P.I. TopSpeed v3.1 for DOS source, using TopSpeed Compatibility Kit ;
all runtime checks are off.
...
(*%E *)
    ...
    IF Lib.ParamCount() # 0 THEN
        ...
        IF ( ok AND (lc # 0) ) THEN
            niter := lc;
        ELSE
			WrStr("FBENCH32 (Excelsior XDS v2.60beta compiler for Win32)");WrLn;
		    WrLn;
            WrStr("Syntax : FBENCH32 [iterations]");WrLn;
            WrLn;
            WrStr("a) This is John Walker's FBENCH program (found at fourmilab.ch Web site)");WrLn;
            WrStr("   quickly ported by PhG to Excelsior XDS v2.60beta Modula-2 compiler for Win32");WrLn;
			WrStr("   from J.P.I. TopSpeed v3.1 Modula-2 compiler for DOS port.");WrLn;
