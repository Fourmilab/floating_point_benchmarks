    ...
    #=  transitSurface propagates a ray through a Design.
        ...
        The TraceContext ctx provides input and output as
        follows.
    ...
    function transitSurface(
        ...
        radius_of_curvature = d.surf[ctx.cSurf].curvature_Radius;
        to_index = d.surf[ctx.cSurf].index_Of_Refraction;
        ...
        if to_index > 1
            to_index += ((D_line - line) /
                (C_line - F_line)) *
                ((d.surf[ctx.cSurf].index_Of_Refraction - 1) /
                d.surf[ctx.cSurf].dispersion)
        end
        ...
        if axial_incidence == Paraxial_Ray
            ...
            if radius_of_curvature != 0
                ...
                odz = ctx.object_distance == 0;
                asaprime = odz ? 0 : ctx.axis_slope_angle;
                iangsin = odz ? ctx.ray_height / radius_of_curvature :
                                ((ctx.object_distance - radius_of_curvature) /
                                 radius_of_curvature) * ctx.axis_slope_angle;
                rangsin = (ctx.from_index / to_index) * iangsin;
                asadoubleprime = asaprime + iangsin - rangsin;
                rayheightprime = odz ? ctx.ray_height :
                                        ctx.object_distance * asaprime;
                objectdistanceprime = rayheightprime / asadoubleprime;

                return TraceContext(ctx.cSurf + 1,
                    objectdistanceprime - d.surf[ctx.cSurf].edge_Thickness,
                    rayheightprime,
                    asadoubleprime, to_index, 0);
            else

                #   Flat surface

                return TraceContext(ctx.cSurf + 1,
                    (object_distance * (to_index / ctx.from_index)) -
                        d.surf[ctx.cSurf].edge_Thickness,
                    ctx.ray_height,
            ...
            if radius_of_curvature != 0
                ...
                iangsin = odz ? ctx.ray_height / radius_of_curvature :
                                ((ctx.object_distance - radius_of_curvature) /
                                 radius_of_curvature) * sin(ctx.axis_slope_angle);
                iang = asin(iangsin);
                rangsin = (ctx.from_index / to_index) * iangsin;
                ...
                rang = -(asin((ctx.from_index / to_index))) *
                              sin(ctx.axis_slope_angle);

                return TraceContext(ctx.cSurf + 1,
                    (ctx.object_distance * ((to_index *
                            cos(-rang)) / (ctx.from_index *
                            cos(ctx.axis_slope_angle)))) -
                            d.surf[ctx.cSurf].edge_Thickness,
                    ctx.ray_height,
                    -rang,
                    to_index, 0);
    ...
    function traceLine(
        ...
        tc = TraceContext(1, 0, d.clearAperture / 2, 0, 1, 0);

        for i = 1 : length(d.surf)
            tc = transitSurface(axial_incidence, d, line, tc);
        end

        return (tc.object_distance, tc.axis_slope_angle);
    end
